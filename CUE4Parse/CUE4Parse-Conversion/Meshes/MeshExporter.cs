...
using CUE4Parse_Conversion.Meshes.XModel;
    ...
    public class MeshExporter : ExporterBase
    {
        ...
        public MeshExporter(UStaticMesh originalMesh, USplineMeshComponent? splineMeshComponent, ExporterOptions options) : base(originalMesh, options)
        {
            ...
            foreach (var lod in convertedMesh.LODs)
            {
                ...
                string ext;
                XModelExporter? xmodelExporter = null;
                ...
                switch (Options.MeshFormat)
                {
                    ...
                    case EMeshFormat.OBJ:
                        ...
                        new Gltf(ExportName, lod, materialExports, Options).Save(Options.MeshFormat, Ar);
                        break;
                    case EMeshFormat.XModel:
                        ext = "XMODEL_EXPORT";
                        // Get materials from the LOD
                        materialExports = options.ExportMaterials ? convertedMesh.LODs[0].GetMaterials(options) : null;
                        xmodelExporter = new XModelExporter(convertedMesh, ExportName);
                        using (var ms = new MemoryStream())
                        using (var sw = new StreamWriter(ms))
                        {
                            xmodelExporter.Save(sw);
                            sw.Flush();
                            Ar.Write(ms.ToArray());
                        }
                        break;
                ...
                {
                    var mesh = new Mesh($"{path}.{ext}", Ar.GetBuffer(), materialExports ?? new List<MaterialExporter2>(), Options);
                    MeshLods.Add(mesh);
                    break;
                }

                var lodMesh = new Mesh($"{path}_LOD{i}.{ext}", Ar.GetBuffer(), materialExports ?? new List<MaterialExporter2>(), Options);
                MeshLods.Add(lodMesh);
            }
        }
            ...
            for (var lodIndex = 0; lodIndex < convertedMesh.LODs.Count; lodIndex++)
            {
                ...
                switch (Options.MeshFormat)
                {
                    ...
                    case EMeshFormat.XModel:
                        ext = "XMODEL_EXPORT";
                        // Get materials from the LOD
                        materialExports = options.ExportMaterials ? lod.GetMaterials(options) : null;
                        xmodelExporter = new XModelExporter(lod, convertedMesh.RefSkeleton.ToArray(), ExportName);
                        using (var ms = new MemoryStream())
                        using (var sw = new StreamWriter(ms))
                        {
                            xmodelExporter.Save(sw);
                ...
                if (Options.LodFormat == ELodFormat.FirstLod)
                {
                    var mesh = new Mesh($"{GetExportSavePath()}.{ext}", Ar.GetBuffer(), materialExports ?? new List<MaterialExporter2>(), Options);
                    MeshLods.Add(mesh);
                    break;
                }
                else
                {
                    var lodMesh = new Mesh($"{GetExportSavePath()}_LOD{i}.{ext}", Ar.GetBuffer(), materialExports ?? new List<MaterialExporter2>(), Options);
                    MeshLods.Add(lodMesh);
                }
                i++;
            }
